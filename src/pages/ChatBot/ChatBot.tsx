...
import placeholdersRaw from '../../data/chatbotPlaceholders.json';
...
const ChatBot = () => {
  ...
  const [forzarOffline, setForzarOffline] = useState(false); // Toggle manual de modo offline
  const chatRef = useRef<HTMLDivElement>(null);
  const [isMobile, setIsMobile] = useState(false);

  // Detectar tamaño de pantalla para responsive dinámico
  useEffect(() => {
  ...
  const handleEnviar = async (e: React.FormEvent) => {
    ...
    setInput("");
    try {
      const PLACE = placeholdersRaw as string[];

      const buildOfflineAnswer = () => {
        // Solo placeholder aleatorio, sin FAQ ni info extendida
        return PLACE[Math.floor(Math.random()*PLACE.length)];
      };

      // Modo offline forzado por usuario
      if (forzarOffline) {
        const texto = buildOfflineAnswer();
        setMensajes(prev => [...prev, { autor: "ia", texto }]);
        setModoOffline(true);
        setCargando(false);
        return;
      }
      ...
      if (!backendOk) {
        ...
        setMensajes(prev => [...prev, { autor: 'ia', texto }]);
        setModoOffline(true);
        if (!diagnostico) setDiagnostico('Sin conexión al backend. Modo offline.');
        setMostrarCortina(true);
      } else {
        if (data.online === false) {
            // Backend responde pero indica offline/placeholder
            const texto = buildOfflineAnswer();
            setMensajes(prev => [...prev, { autor: 'ia', texto }]);
            setModoOffline(true);
            if (data.reason === 'missing_api_key') {
              setDiagnostico('Backend sin clave: usando modo offline.');
              setMostrarCortina(true);
            } else if (data.reason === 'placeholder_mode') {
              if (!diagnostico) setDiagnostico('Modo mantenimiento activo.');
            }
        } else {
      ...
      const PLACE = placeholdersRaw as string[];
      const elegido = PLACE[Math.floor(Math.random()*PLACE.length)];
      setMensajes(prev => [...prev, { autor: 'ia', texto: elegido }]);
      setModoOffline(true);
      if (!diagnostico) setDiagnostico('Error inesperado. Modo offline.');
    }
    setCargando(false);
  };
  ...
  return (
    <div className="h-screen text-white overflow-hidden flex flex-col" style={{ background: 'radial-gradient(ellipse at top, #1a1a1a 0%, #2a2a2a 30%, #0f0f0f 60%, #000000 100%)' }}>
      ...
      {mostrarCortina && (
        <div className="absolute inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md overflow-y-auto">
          <div className="relative w-full max-w-xl bg-[#1a1a1a]/70 border border-[#FFD700]/30 rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-2xl">
            ...
            <div className="relative">
              <div className="flex items-center gap-3 sm:gap-4 mb-4">
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-[#FFD700] to-[#C9B037] flex items-center justify-center text-xl sm:text-2xl shadow-lg">🤖</div>
                <h2 className="text-xl sm:text-2xl font-bold text-[#FFD700] tracking-wide">Asistente en Modo Limitado</h2>
              </div>
              <p className="text-gray-300 leading-relaxed mb-4 text-sm sm:text-sm">
              ...
              <ul className="text-gray-300 text-xs sm:text-sm space-y-2 mb-5 sm:mb-6 list-disc pl-5">
                ...
                <li>Puedes activar modo offline manual aunque el backend responda.</li>
              </ul>
              ...
              <div className="mt-5 flex items-center justify-between gap-4">
                <label className="flex items-center gap-2 text-xs text-gray-300 cursor-pointer select-none">
                  <input type="checkbox" className="accent-[#FFD700]" checked={forzarOffline} onChange={e => { setForzarOffline(e.target.checked); if (e.target.checked) { setModoOffline(true); } }} />
                  Forzar modo offline manual
                </label>
                {forzarOffline && <span className="text-[10px] text-[#FFD700]">Respuestas locales</span>}
              </div>
              {modoOffline && (
  ...
  <div className={`fixed inset-0 pointer-events-none ${isMobile ? 'opacity-60' : ''}`}> 
  ...
  <div className="relative py-4 px-4 flex-shrink-0">
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-3 sm:py-4 bg-[#1a1a1a]/40 backdrop-blur-sm rounded-2xl border border-[#FFD700]/20 shadow-lg relative w-full sm:w-auto max-w-md sm:max-w-none">
              ...
              <div className="text-left">
                <h3 className="text-[#FFD700] font-semibold text-sm sm:text-base leading-tight">
                  Asistente CFIM
                </h3>
              ...
              <button
                type="button"
                onClick={() => setMostrarCortina(true)}
                className="absolute -top-2 -right-2 w-7 h-7 rounded-full bg-[#FFD700]/80 text-black text-xs font-bold shadow hover:scale-110 transition"
                title="Estado / Modo"
              >i</button>
            </div>
          </div>
          {modoOffline && (
            <div className="mt-3 flex justify-center">
              <span className="px-3 py-1 rounded-full text-[10px] sm:text-xs font-semibold bg-[#FFD700]/20 text-[#FFD700] border border-[#FFD700]/40 backdrop-blur-sm">Modo offline</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto py-4 space-y-6">
            {mensajes.map((msg, idx) => (
              ...
              >
        <div className={`relative ${isMobile ? 'max-w-[90%]' : 'max-w-[75%]'} ${
                  msg.autor === "user"
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {!primeraConsulta && (
              <div className="mb-4 -mx-1">
                <div className="flex sm:grid sm:grid-cols-4 gap-2 sm:gap-3 overflow-x-auto scrollbar-thin px-1 pb-1 sm:overflow-visible">
                  {sugerencias.map((s, i) => (
                    <button
                      key={i}
                      className="relative group flex-shrink-0 p-3 bg-[#1a1a1a]/40 backdrop-blur-md rounded-xl border border-[#FFD700]/20 hover:border-[#FFD700]/50 transition-all duration-300 hover:scale-105 text-left disabled:opacity-50 shadow-xl overflow-hidden min-w-[70%] sm:min-w-0"
                    ...
                    >
                      ...
                      <span className="text-gray-300 group-hover:text-[#FFD700] transition-colors duration-300 font-medium text-xs sm:text-sm">
                        {s}
                      </span>
                    </button>
                  ))}
            ...
            <form className="flex gap-3" onSubmit={handleEnviar}>
              <div className="flex-1 relative group">
                ...
                <input
                  type="text"
                  className="relative w-full rounded-2xl px-4 sm:px-6 py-3 sm:py-4 bg-[#1a1a1a]/60 backdrop-blur-md text-white border border-[#FFD700]/30 focus:outline-none focus:border-[#FFD700] focus:ring-2 focus:ring-[#FFD700]/20 transition-all duration-300 pr-14 placeholder-gray-400 font-medium shadow-xl text-sm sm:text-base"
                  placeholder={isMobile ? "Escribe tu pregunta..." : "Pregúntame sobre la carrera de Metalurgia..."}
                  value={input}
