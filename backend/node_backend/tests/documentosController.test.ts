...
import * as controller from '../src/controllers/documentosController';

vi.mock('../src/services/supabaseService', () => ({
  listTable: vi.fn(async () => ({ items: [{ id: '1', titulo: 'Reglamento', tipo: 'pdf' }], total: 1, page: 1, pageSize: 20 })),
  getById: vi.fn(async (_t: string, id: string) => id === '1' ? ({ id: '1', titulo: 'Reglamento', tipo: 'pdf' }) : null),
...
describe('documentosController', () => {
  ...
  it('lista documentos', async () => { const res = mockRes(); await controller.listar({ query:{} } as any, res, noopNext); expect(res.jsonData.total).toBe(1); });
});
